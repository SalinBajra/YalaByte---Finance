...
export default function FinanceApp({ profile, signOut }) {
  ...
  useEffect(() => {
    ...
    ]).then(([dealResult, invoiceResult, eventResult, paymentResult, payrollEventResult, teamResult]) => {
  ...
  const voidRow = async (row) => {
    ...
    await auditPayroll(mapped, 'voided', 'Payroll entry voided and hidden from active financial views.');
    return true;
  };
