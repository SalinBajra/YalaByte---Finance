...
function FinanceLoader({ profile, compact = false }) {
  ...
  return <main className="finance-shell flex min-h-screen items-center justify-center px-5 text-white"><div className="text-center"><div className="flex justify-center"><Brand inverted /></div><span className="mx-auto mt-10 flex h-16 w-16 items-center justify-center rounded-full border border-cyanbrand-400/30 bg-white/10 text-lg font-extrabold text-cyanbrand-400 shadow-[0_0_40px_rgba(19,200,222,0.15)]">{initials}</span><p className="mt-6 text-xs font-extrabold uppercase tracking-[0.2em] text-cyanbrand-400">Finance workspace</p><h1 className="mt-3 text-2xl font-extrabold tracking-tight sm:text-3xl">Welcome, {label.split(/\s|@/)[0]}</h1><p className="mt-2 text-sm text-slate-300">Your YalaByte finance workspace is getting everything ready.</p><span className="mx-auto mt-6 block h-1 w-24 overflow-hidden rounded-full bg-white/10"><span className="block h-full w-1/2 animate-pulse rounded-full bg-cyanbrand-400" /></span></div></main>;
}
...
export default function AuthGate({ children }) {
  ...
  if (state.loading) return <main className="finance-shell grid min-h-screen place-items-center text-white"><p className="text-sm font-semibold text-slate-300">Opening secure Finance…</p></main>;
