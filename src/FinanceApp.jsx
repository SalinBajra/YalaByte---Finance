...
const isPayrollRow = (row) => row.type === 'expense'
  && row.status !== 'Void'
...
function Payroll({ rows, teamMembers, onAdd, onUpdate, onDelete, error }) {
  ...
  return <><div><h1 className="text-3xl font-semibold tracking-tight text-ink">Payroll</h1><p className="mt-2 text-slate-500">Record salary, contractor, and benefit payments without mixing payroll into the client/vendor transaction form.</p></div><section className="mt-7 grid gap-4 md:grid-cols-3"><Metric label="Payroll paid" value={money(total)} note={`${payrollRows.length} active payroll entries`} tone="text-emerald-600" /><Metric label="Next cycle" value="Monthly" note="Post payroll below" /><Metric label="Corrections" value="Edit / void" note="Keep finance records traceable" tone="text-amber-600" /></section><form onSubmit={submit} className="mt-5 grid gap-3 rounded-2xl border border-slate-200 bg-white p-5 md:grid-cols-2 xl:grid-cols-6"><label className="text-xs font-bold uppercase tracking-wider text-slate-500 xl:col-span-2">Employee or contractor<select value={form.payee} onChange={(event) => setForm({ ...form, payee: event.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-medium normal-case tracking-normal outline-none focus:border-cyanbrand-500"><option value="">Select team member</option>{teamMembers.map((member) => <option key={member.user_id || member.email || member.name} value={member.name}>{member.name}{member.role ? ` - ${member.role}` : ''}</option>)}</select></label><label className="text-xs font-bold uppercase tracking-wider text-slate-500">Pay period<input value={form.period} onChange={(event) => setForm({ ...form, period: event.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-medium normal-case tracking-normal outline-none focus:border-cyanbrand-500" placeholder="June 2026" /></label><label className="text-xs font-bold uppercase tracking-wider text-slate-500">Payroll type<select value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm normal-case"><option value="Payroll">Salary</option><option value="Contractor">Contractor</option><option value="Benefits">Benefits</option><option value="Bonus">Bonus</option></select></label><label className="text-xs font-bold uppercase tracking-wider text-slate-500">Amount (Rs)<input type="number" min="0" value={form.amount} onChange={(event) => setForm({ ...form, amount: event.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-medium normal-case tracking-normal outline-none focus:border-cyanbrand-500" /></label><div className="flex items-end gap-2"><button disabled={saving || !teamMembers.length} className="w-full rounded-xl bg-cyanbrand-500 px-4 py-3 text-sm font-bold text-ink disabled:opacity-60">{saving ? 'Saving...' : editingId ? 'Save changes' : 'Record payroll'}</button>{editingId ? <button type="button" onClick={resetForm} className="rounded-xl border border-slate-200 px-4 py-3 text-sm font-bold text-slate-600 hover:bg-slate-50">Cancel</button> : null}</div><label className="text-xs font-bold uppercase tracking-wider text-slate-500 md:col-span-2 xl:col-span-6">Internal note<input value={form.note} onChange={(event) => setForm({ ...form, note: event.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-medium normal-case tracking-normal outline-none focus:border-cyanbrand-500" placeholder="Optional payroll note" /></label></form>{!teamMembers.length ? <p className="mt-3 rounded-xl bg-amber-50 p-3 text-sm font-medium text-amber-800">No approved team members are available for payroll yet.</p> : null}{error && <p className="mt-4 rounded-xl bg-red-50 p-3 text-sm font-medium text-red-700">{error}</p>}<section className="mt-5 rounded-2xl border border-slate-200 bg-white"><div className="border-b border-slate-100 p-5"><h2 className="font-semibold text-ink">Payroll activity</h2><p className="mt-1 text-sm text-slate-500">Edit mistakes or void test entries without erasing the financial trail.</p></div>{payrollRows.length ? <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left"><thead><tr className="text-xs uppercase tracking-wider text-slate-400">{['Date','Employee','Payroll detail','Status','Amount','Actions'].map((heading) => <th key={heading} className="px-5 py-3 font-bold">{heading}</th>)}</tr></thead><tbody>{payrollRows.map((row) => <tr key={row.id} className="border-t border-slate-100 text-sm"><td className="whitespace-nowrap px-5 py-4 text-slate-500">{date(row.date)}</td><td className="px-5 py-4 font-semibold text-ink">{row.party}</td><td className="px-5 py-4"><p className="font-semibold text-ink">{row.description}</p><p className="mt-1 text-xs text-slate-400">{row.category}</p></td><td className="px-5 py-4"><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-bold text-emerald-700">{row.status}</span></td><td className="px-5 py-4 text-right font-bold text-ink">{money(row.amount)}</td><td className="px-5 py-4"><div className="flex flex-wrap gap-2"><button type="button" onClick={() => editRow(row)} className="rounded-lg border border-slate-200 px-3 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50">Edit</button><button type="button" onClick={() => onDelete(row)} className="rounded-lg bg-rose-50 px-3 py-2 text-xs font-bold text-rose-700 hover:bg-rose-100">Delete</button></div></td></tr>)}</tbody></table></div> : <p className="p-8 text-center text-sm text-slate-500">No payroll entries yet. Record one above.</p>}</section></>;
}
...
export default function FinanceApp({ profile, signOut }) {
  ...
  const updateRow = async (id, form) => {
    ...
    if (error) { setDataError(error.message); return false; }
    setRows((current) => current.map((row) => row.id === id ? mapTransaction(data) : row));
    return true;
  };
  const voidRow = async (row) => {
    if (!window.confirm(`Delete this payroll entry for ${row.party || 'this team member'}? It will be voided and hidden from active payroll.`)) return false;
    setDataError('');
    const { data, error } = await supabase.from('finance_transactions').update({ status: 'void' }).eq('id', row.id).select().single();
    if (error) { setDataError(error.message); return false; }
    setRows((current) => current.map((item) => item.id === row.id ? mapTransaction(data) : item));
    return true;
  };
