...
function Payroll({ rows, teamMembers, onAdd, error }) {
  ...
  return <><div><h1 className="text-3xl font-semibold tracking-tight text-ink">Payroll</h1><p className="mt-2 text-slate-500">Record salary, contractor, and benefit payments without mixing payroll into the client/vendor transaction form.</p></div><section className="mt-7 grid gap-4 md:grid-cols-3"><Metric label="Payroll paid" value={money(total)} note={`${payrollRows.length} payroll entries`} tone="text-emerald-600" /><Metric label="Next cycle" value="Monthly" note="Post payroll below" /><Metric label="Open items" value="0" note="No approval queue yet" tone="text-amber-600" /></section><form onSubmit={submit} className="mt-5 grid gap-3 rounded-2xl border border-slate-200 bg-white p-5 md:grid-cols-2 xl:grid-cols-6"><label className="text-xs font-bold uppercase tracking-wider text-slate-500 xl:col-span-2">Employee or contractor<select value={form.payee} onChange={(event) => setForm({ ...form, payee: event.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-medium normal-case tracking-normal outline-none focus:border-cyanbrand-500"><option value="">Select team member</option>{teamMembers.map((member) => <option key={member.user_id || member.email || member.name} value={member.name}>{member.name}{member.role ? ` - ${member.role}` : ''}</option>)}</select></label><label className="text-xs font-bold uppercase tracking-wider text-slate-500">Pay period<input value={form.period} onChange={(event) => setForm({ ...form, period: event.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-medium normal-case tracking-normal outline-none focus:border-cyanbrand-500" placeholder="June 2026" /></label><label className="text-xs font-bold uppercase tracking-wider text-slate-500">Payroll type<select value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm normal-case"><option value="Payroll">Salary</option><option value="Contractor">Contractor</option><option value="Benefits">Benefits</option><option value="Bonus">Bonus</option></select></label><label className="text-xs font-bold uppercase tracking-wider text-slate-500">Amount (Rs)<input type="number" min="0" value={form.amount} onChange={(event) => setForm({ ...form, amount: event.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-medium normal-case tracking-normal outline-none focus:border-cyanbrand-500" /></label><button disabled={saving || !teamMembers.length} className="self-end rounded-xl bg-cyanbrand-500 px-4 py-3 text-sm font-bold text-ink disabled:opacity-60">{saving ? 'Saving...' : 'Record payroll'}</button><label className="text-xs font-bold uppercase tracking-wider text-slate-500 md:col-span-2 xl:col-span-6">Internal note<input value={form.note} onChange={(event) => setForm({ ...form, note: event.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-medium normal-case tracking-normal outline-none focus:border-cyanbrand-500" placeholder="Optional payroll note" /></label></form>{!teamMembers.length ? <p className="mt-3 rounded-xl bg-amber-50 p-3 text-sm font-medium text-amber-800">No approved team members are available for payroll yet.</p> : null}{error && <p className="mt-4 rounded-xl bg-red-50 p-3 text-sm font-medium text-red-700">{error}</p>}<section className="mt-5 rounded-2xl border border-slate-200 bg-white"><div className="border-b border-slate-100 p-5"><h2 className="font-semibold text-ink">Payroll activity</h2><p className="mt-1 text-sm text-slate-500">Entries tagged as payroll, salary, contractor, bonus, or benefits.</p></div>{payrollRows.length ? <TransactionTable rows={payrollRows} /> : <p className="p-8 text-center text-sm text-slate-500">No payroll entries yet. Record one above.</p>}</section></>;
}
